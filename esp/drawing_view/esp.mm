...
// ===== THÊM MỚI: Path cho box bị che khuất =====
b.boxBotHiddenPath    = CGPathCreateMutable();
b.boxPlayerHiddenPath = CGPathCreateMutable();
// ==============================================
...
b.linePlayerPath = CGPathCreateMutable();
    ...
    b.boxBotHiddenDirty = b.boxPlayerHiddenDirty = NO; // THÊM MỚI
...
static inline void ESPGeometryBuffersRelease(ESPGeometryBuffers *b) {
    ...
    // Giải phóng path mới
    ESPReleasePath(b->boxBotHiddenPath);
    ESPReleasePath(b->boxPlayerHiddenPath);
...
// ===== THÊM MỚI: Layer cho box bị che khuất =====
@property(nonatomic,strong) CAShapeLayer *boxBotHiddenLayer;
@property(nonatomic,strong) CAShapeLayer *boxPlayerHiddenLayer;
// ==============================================
...
// ===== THÊM MỚI: Layer cho box bị che khuất (màu đỏ) =====
self.boxBotHiddenLayer =
[self makeShapeLayer:red
                fill:nil
           lineWidth:0.7f
                zPos:3];

self.boxPlayerHiddenLayer =
[self makeShapeLayer:red
                fill:nil
           lineWidth:0.7f
                zPos:3];
...
// ======== FOV - ĐÃ SỬA MÀU TÍM HỒNG & TO HƠN ========
UIColor *pinkPurple = [UIColor colorWithRed:1.0f green:0.0f blue:0.5f alpha:1.0f]; // #FF0080
self.fovLayer = [self makeShapeLayer:pinkPurple
                                fill:nil
                           lineWidth:2.0f  // tăng từ 0.7 lên 2.0
                                zPos:0];
// =====================================================
    ...
    for (CAShapeLayer *l in @[self.fovLayer, self.snaplineLayer, self.boneLayer, self.boxLayer,
                            ...
                            self.boxBotHiddenLayer, self.boxPlayerHiddenLayer]) // THÊM LAYER MỚI
...
// ===== THÊM MỚI: Apply path cho box bị che khuất =====
ApplyPath(self.boxBotHiddenLayer,
          buffers.boxBotHiddenPath,
          buffers.boxBotHiddenDirty);

ApplyPath(self.boxPlayerHiddenLayer,
          buffers.boxPlayerHiddenPath,
          buffers.boxPlayerHiddenDirty);
// =====================================================
...
// ======== HIỂN THỊ SỐ ĐỊCH - ĐÃ SỬA FONT NHỎ ========
if (isCount) {
    ...
    if (totalEnemies == 0) {
        ...
        // ========== "Người chơi : X" với cỡ chữ nhỏ gọn ==========
        self.statusLayer.string =
        [NSString stringWithFormat:@"Người chơi : %d", totalEnemies];
        self.statusLayer.foregroundColor = [UIColor whiteColor].CGColor;
        self.statusLayer.fontSize = 14.0f;   // cỡ nhỏ
        self.statusLayer.font = (__bridge CFTypeRef)[UIFont systemFontOfSize:14.0f weight:UIFontWeightBold];
        self.statusLayer.frame =
        CGRectMake(vw / 2.0f - 80.0f,
                   30.0f,      // đưa lên cao
                   160.0f,
                   22.0f);
        self.statusLayer.hidden = NO;
...
- (void)clearAllContent {
    ...
    // ===== THÊM MỚI: Xóa path của layer ẩn =====
    self.boxBotHiddenLayer.path = nil;
    self.boxPlayerHiddenLayer.path = nil;
    // ===========================================
...
if (isBot)
    ...
    stats.playerCount++;
            ...
            // ===== SỬA ĐỔI: Truyền thêm aimVis vào RenderESPForPawn =====
